# COPO Vision Authorization Policy v1.0.3
...
| **1.0.2** | 2026-09-21 | Decoupled evidence status from implementation status; decoupled Admin role existence from unverified institution scope; distinguished confirmed BOLA vulnerabilities from proposed remediation; established dedicated Function-Level RBAC Decision IDs (`AUTH-027`, `AUTH-028`, `AUTH-029`). | Pre-review stabilization pass. | Draft |
| **1.0.3** | 2026-09-21 | **Decision Register & Permission Matrix Reconciliation**: Explicitly structured the Decision Register into *Granular Permission Decisions*, *Cross-Cutting Architectural Policies*, and *Domain-Model Requirements*; consolidated redundant prediction decision IDs (`AUTH-017` consolidated into `AUTH-025`); decoupled overloaded Admin Scope (`AUTH-003`) by creating dedicated Master Catalog Reference Visibility decision (`AUTH-030`); corrected catalog read implementation status to reflect that object-level authorization is not implemented despite 5A route existence. | Final pre-review reconciliation pass establishing strict semantic accuracy and eliminating all register-matrix mismatches. | **DRAFT / PENDING BUSINESS APPROVAL** |
...
| **Master Catalog Read Scope** | `CONFIRMED` (Routes return all departments/courses/subjects) | Open reference read for authenticated users. | `PROPOSED` | **NOT IMPLEMENTED** | Business Approval |
...
* **`NOT IMPLEMENTED`**: No runtime enforcement exists in the active codebase. (Note: A route being reachable under global 5A authentication does not mean object- or scope-level authorization is implemented).
* **`NOT APPLICABLE`**: Operation not supported or not pertinent to the resource.

---

## 2. Decision Register vs. Permission Matrix Architecture

To establish enterprise-grade traceability without artificial constraints:

1. **Permission Matrix**: Every row represents a discrete, actionable operation on a specific academic resource and maps to an explicit Decision ID in the Decision Register.
2. **Decision Register**: Contains all authorization decisions, categorized into three distinct types:
   - **Type A: Granular Permission Decisions** (Directly mapped 1-to-1 with Permission Matrix rows, e.g. `AUTH-009`, `AUTH-021`, `AUTH-027`).
   - **Type B: Cross-Cutting Architectural Policies** (System-wide rules governing multiple operations, e.g. `AUTH-006` Academic Year Context, `AUTH-007` Semester Context, `AUTH-008` Section Scope, `AUTH-010` Assessment Ownership, `AUTH-011` Historical Access, `AUTH-020` Security Audit Logging, `AUTH-024` Co-Teaching Hierarchy).
   - **Type C: Domain-Model Requirements** (Prerequisite relational structures required to enable authorization, e.g. `AUTH-001` Faculty Scope, `AUTH-002` HOD Scope, `AUTH-004` Assignment Entity).
3. **Traceability Rule**: *Every Permission Matrix row maps to an explicit Decision ID in the Decision Register.* Cross-cutting architectural decisions do not require redundant CRUD matrix rows; they are cross-referenced across the document and govern contextual constraints across multiple operations.

---

## 3. Policy Principles

The COPO Vision Authorization Framework shall enforce the following twelve mandatory enterprise principles upon implementation:
...
## 4. Strict Implementation Dependency Lifecycle
...
* **Function-Level RBAC**: `CONFIRMED` & `IMPLEMENTED` for `/api/copo/calculate`, `/api/predictions/generate`, `/api/alerts/generate` (Decisions `AUTH-027`, `AUTH-028`, `AUTH-029`).
...
## 6. Organizational Hierarchy & Attachment Points
...
## 9. Definitive Permission Matrix
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Departments** | READ | ALLOW | ALLOW | ALLOW | Institution | None | `PROPOSED` | AUTH-030 | **NOT IMPLEMENTED** |
| **Departments** | CREATE / EDIT | ALLOW | DENY | DENY | Institution | Admin Only | `PROPOSED` | AUTH-003 | **NOT IMPLEMENTED** |
| **Courses** | READ | ALLOW | ALLOW | ALLOW | Institution | None | `PROPOSED` | AUTH-030 | **NOT IMPLEMENTED** |
| **Courses** | CREATE / EDIT | ALLOW | ALLOW (Dept) | DENY | Department | Academic Year | `BUSINESS DECISION REQUIRED` | AUTH-026 | **NOT IMPLEMENTED** |
| **Subjects** | READ | ALLOW | ALLOW | ALLOW | Institution | None | `PROPOSED` | AUTH-030 | **NOT IMPLEMENTED** |
...
## 10. Marks Upload Security Policy (`POST /api/uploads/marks`)

### 10.1 Vulnerability Finding vs. Remediation Policy
...
### 10.2 Proposed Denial Behavior (Upon Implementation)
If an authenticated faculty member attempts to submit marks for an unassigned assessment:
* The transaction will be aborted immediately before file processing or database insertion.
* The server will respond with `HTTP 403 Forbidden` (`{"status": "error", "message": "Access denied: You are not assigned to this subject/assessment"}`).

---

## 11. Assessment Ownership & Lifecycle Policy
...
## 13. Recommended Data-Scope Principle
...
## 14. HOD Authorization Policy

* **Departmental Jurisdiction (`PROPOSED` — Decision `AUTH-002`)**: An HOD's authority is proposed to be strictly bounded by their assigned `department_id` (`DOMAIN MODEL REQUIRED`; `NOT IMPLEMENTED`).
* **Proposed Supervisory Read Rights**: Upon implementation, HOD users will be authorized to inspect all student marks, assessment blueprints, attainment calculations, and alerts across all courses within their assigned department.
* **Proposed Supervisory Write Rights**: Upon implementation, HOD users will be authorized to trigger batch calculations and generate alerts for their assigned department.
* **Proposed Cross-Department Denial**: Upon implementation, HOD users will have zero administrative or write privileges over courses or subjects housed in other academic departments.

---

## 15. Admin Authorization Policy

* **Role & Function Authority (`CONFIRMED` — Decision `AUTH-027`, `AUTH-028`, `AUTH-029`)**: Admin role is active in the database and enforced on global function triggers.
...
* **Audit Transparency (`PROPOSED` — Decision `AUTH-020`)**: Upon implementation, all administrative write and override actions must be logged to immutable security audit logs.

---

## 16. Alert Authorization Policy & `faculty_id = NULL` Resolution

### 16.1 Alert Scoping Taxonomy (`PROPOSED` — Decision `AUTH-018`)
...
### 16.2 Resolution of `faculty_id = NULL` (`BUSINESS DECISION REQUIRED` — Decision `AUTH-018`)
...
## 18. Academic Context Policy (Year & Semester)
...
## 22. Server-Side Enforcement Invariant

All authorization logic resides on the server. The Flask application currently verifies `g.current_user["role"]` for implemented RBAC endpoints, and the Flask application must verify relational assignment bindings once the required domain model is implemented, before calling services or executing SQL queries.

---

## 23. Client-Controlled Object IDs (BOLA Defense)
...
```
Upon implementation, if caller context fails to match the entity's resolved parent scope, access will be denied.

---

## 24. Auditability & Security Logging
...
## 25. Authorization Failure Semantics
...
## 26. Complete Policy Decision Register

The Policy Decision Register defines all domain decisions, categorized by **Decision Type**:
* **Granular Permission Decisions** (direct matrix mappings),
* **Cross-Cutting Architectural Policies** (system-wide governing rules),
* **Domain-Model Requirements** (prerequisite schema entities).

| ID | Decision Area | Decision Type | Existing Evidence | Proposed Enterprise Policy | Policy Status | Business Approval Status | Domain Model Req | Implementation Status | Next Action |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`AUTH-001`** | Faculty Scope | Domain Model Req | `users.role = 'faculty'` exists; no assignment table. | Bounded strictly to active assigned subjects. | `PROPOSED` | `PROPOSED` | **REQUIRED** (`faculty_subject_assignments`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-002`** | HOD Scope | Domain Model Req | `users.role = 'hod'` exists; `users` lacks `department_id`. | Bounded strictly to assigned department. | `PROPOSED` | `PROPOSED` | **REQUIRED** (`users.department_id`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-003`** | Admin Master Scope | Granular Permission | `users.role = 'admin'` exists; function RBAC verified. | Institution-wide administrative mutation authority. | `PROPOSED` | `PROPOSED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-004`** | Faculty Assignment Entity | Domain Model Req | Absent from schema; no table links user to subject. | Intermediate relation mapping `user_id` to `subject_id`. | `PROPOSED` | `PROPOSED` | **REQUIRED** (`faculty_subject_assignments`) | **NOT IMPLEMENTED** | Schema Design |
| **`AUTH-005`** | Multiple Faculty per Subject | Cross-Cutting Arch | Absent from schema. | Allowed (co-instructors / lab evaluators). | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-006`** | Academic Year Context | Cross-Cutting Arch | `academic_years` table exists in MySQL export. | Assignments scoped to `academic_year_id`. | `PROPOSED` | `PROPOSED` | **REQUIRED** | **NOT IMPLEMENTED** | Schema Design |
| **`AUTH-007`** | Semester Context | Cross-Cutting Arch | `subjects.semester` column exists in schema. | Assignments scoped to `subjects.semester`. | `PROPOSED` | `PROPOSED` | **REQUIRED** | **NOT IMPLEMENTED** | Schema Design |
| **`AUTH-008`** | Section / Division Scope | Cross-Cutting Arch | Absent from active schema. | Subdivide subject assignments by section. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** (`sections` table) | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-009`** | Marks Upload Authorization | Granular Permission | `assessment_id` unverified (`CONFIRMED BOLA GAP`). | Restricted strictly to active assigned faculty. | `PROPOSED` | `PROPOSED` | **REQUIRED** (`faculty_subject_assignments`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-010`** | Assessment Ownership | Cross-Cutting Arch | `assessments.subject_id` exists; no faculty link. | Subject-level ownership with evaluator rights. | `PROPOSED` | `PROPOSED` | None | **NOT IMPLEMENTED** | Architecture Sign-Off |
| **`AUTH-011`** | Historical Data Access | Cross-Cutting Arch | No temporal read/write query restrictions. | Historical read allowed; past write locked out. | `PROPOSED` | `BUSINESS DECISION REQUIRED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-012`** | Student Directory Visibility | Granular Permission | `GET /api/students/` returns all students. | Open catalog read vs. department filtered. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** (if filtered) | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-013`** | Assessment Visibility | Granular Permission | `GET /api/assessments/` returns all assessments. | Visible only to assigned faculty & department HOD. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-014`** | Marks Read Visibility | Granular Permission | `student_marks` queries unscoped by user. | Visible only to assigned faculty & department HOD. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-015`** | CO Attainment Visibility | Granular Permission | `GET /api/copo/attainment` returns all COs. | Assigned faculty for subject; Dept HOD. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-016`** | PO Attainment Visibility | Granular Permission | `GET /api/copo/po-attainment` returns all POs. | Department HOD & Admin; Faculty subject-derived. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-017`** | *Consolidated* | *Retired* | Redundant with `AUTH-025`. | Consolidated into `AUTH-025` (Student Prediction Read). | `RETIRED` | `RETIRED` | N/A | **N/A** | See `AUTH-025` |
| **`AUTH-018`** | Alert Routing & NULL Semantics| Granular Permission | All 1,989 seed alert rows have `faculty_id = NULL`. | Route system alerts by `subject_id` / `dept_id`. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** (`alerts.subject_id`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-019`** | Dashboard Scoping | Granular Permission | `GET /api/dashboard/summary` computes global KPIs. | Aggregate KPIs strictly to user's authorized domain. | `PROPOSED` | `PROPOSED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-020`** | Security Audit Logging | Cross-Cutting Arch | Unstructured print/console logs only. | Structured JSON audit logging for mutations. | `PROPOSED` | `PROPOSED` | None | **NOT IMPLEMENTED** | Implementation Design |
| **`AUTH-021`** | Faculty Assessment Create/Edit| Granular Permission | Unscoped route execution. | Faculty can create/edit assessments for assigned subjects. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-022`** | Assessment Delete Rights | Granular Permission | No DELETE route in Flask backend. | Soft-archive only; restricted to HOD/Admin. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-023`** | Export Permissions | Granular Permission | No export route in Flask backend. | Mark sheets exportable by assigned faculty/HOD. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-024`** | Co-Teaching Rights Hierarchy | Cross-Cutting Arch | Absent from active runtime. | Primary instructor vs. co-evaluator write rights. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** (`assignment_role`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-025`** | Student Prediction Read Scope | Granular Permission | `GET /api/predictions/` unscoped by user. | Faculty can view predictions only for enrolled students. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | **REQUIRED** (`enrollments`) | **NOT IMPLEMENTED** | Stakeholder Sign-Off $\to$ Model |
| **`AUTH-026`** | Course & Subject Creation | Granular Permission | No CRUD endpoints in active Flask routes. | HOD (own department) and Admin only. | `UNKNOWN` | `BUSINESS DECISION REQUIRED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |
| **`AUTH-027`** | Attainment Calculation RBAC | Granular Permission | Route guarded by `@require_roles('admin', 'hod')`. | Restrict attainment re-computation to Admin/HOD. | `CONFIRMED` | `CONFIRMED` | None | **IMPLEMENTED (5B.2-A)** | Closed / Verified |
| **`AUTH-028`** | Prediction Generation RBAC | Granular Permission | Route guarded by `@require_roles('admin', 'hod')`. | Restrict ML model training to Admin/HOD. | `CONFIRMED` | `CONFIRMED` | None | **IMPLEMENTED (5B.2-A)** | Closed / Verified |
| **`AUTH-029`** | Alert Generation RBAC | Granular Permission | Route guarded by `@require_roles('admin', 'hod')`. | Restrict automated alert creation to Admin/HOD. | `CONFIRMED` | `CONFIRMED` | None | **IMPLEMENTED (5B.2-A)** | Closed / Verified |
| **`AUTH-030`** | Master Catalog Reference Read | Granular Permission | Routes return all departments/courses/subjects. | Open reference visibility across authenticated users. | `PROPOSED` | `PROPOSED` | None | **NOT IMPLEMENTED** | Stakeholder Sign-Off |

---

## 27. Final Conceptual Authorization Model
...
```
             ...
             (Proposed Global Scope)                   │
                                        ┌──────────────┴──────────────┐
                                        ▼                             ▼
                                   [HOD ACTOR]                 [COURSE ENTITY]
                           (Proposed Department Scope)                │
                                                                      ▼
                                                               [SUBJECT ENTITY]
                                                                      │
                                                       ┌──────────────┴──────────────┐
...
## 28. Future Implementation Requirements (Post-Approval Pre-requisites)
...
*The policy specification v1.0.3 is reconciled, semantically precise, and fully traceable. Formal stakeholder sign-off on the Policy Decision Register (`AUTH-001` through `AUTH-030`) and domain model specification are mandatory pre-requisites before writing application code or database migrations.*

---

## 31. Scope Control Confirmation
