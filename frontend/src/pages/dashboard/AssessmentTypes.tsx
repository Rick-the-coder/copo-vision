...
const AssessmentTypesPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['assessment-types'], queryFn: async () => (await api.get('/assessment-types')).data as AssessmentType[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/assessment-types', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-types'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/assessment-types/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-types'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/assessment-types/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-types'] }); }
  });

  const openModal = (item: AssessmentType | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800">Assessment Types</h2>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Assessment Type</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Name</label><input {...register('name', {required: true})} placeholder="e.g., Mid Semester" className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">Description</label><input {...register('description')} className="w-full px-3 py-2 border rounded-lg" /></div>
