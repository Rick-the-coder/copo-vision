...
const FacultyPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['faculty'], queryFn: async () => (await api.get('/faculty')).data as Faculty[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/faculty', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['faculty'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/faculty/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['faculty'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/faculty/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['faculty'] }); }
  });

  const openModal = (item: Faculty | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
              ...
              {isLoading ? <tr><td colSpan={5} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4">{item.faculty_name}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Faculty</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Name</label><input {...register('faculty_name', {required: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">Email</label><input type="email" {...register('email', {required: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">Phone</label><input {...register('phone')} className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">Designation</label><input {...register('designation')} className="w-full px-3 py-2 border rounded-lg" /></div>
