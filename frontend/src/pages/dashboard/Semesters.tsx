...
const SemestersPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['semesters'], queryFn: async () => (await api.get('/semesters')).data as Semester[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/semesters', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['semesters'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/semesters/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['semesters'] }); closeModal(); }
  });
  ...
  const openModal = (item: Semester | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
              ...
              {isLoading ? <tr><td colSpan={3} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4">{item.semester_number}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Semester</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Semester Number</label><input type="number" {...register('semester_number', {required: true, valueAsNumber: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
