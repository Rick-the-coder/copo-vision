...
const CurriculumsPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['curriculums'], queryFn: async () => (await api.get('/curriculums')).data as Curriculum[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/curriculums', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['curriculums'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/curriculums/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['curriculums'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/curriculums/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['curriculums'] }); }
  });

  const openModal = (item: Curriculum | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800">Curriculums</h2>
              ...
              {isLoading ? <tr><td colSpan={5} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4">{item.curriculum_name}</td>
                    <td className="p-4">{item.version}</td>
                    <td className="p-4">{item.department_id}</td>
                    <td className="p-4">{item.program_id}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Curriculum</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Name</label><input {...register('curriculum_name', {required: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
