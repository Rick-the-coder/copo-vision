...
const AssessmentsPage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/assessments', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessments'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/assessments/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessments'] }); closeModal(); }
  });
              ...
              {isLoading ? <tr><td colSpan={6} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4 font-semibold">{item.assessment_name}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Assessment</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Assessment Name</label><input {...register('assessment_name', {required: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm mb-1">Max Marks</label><input type="number" {...register('maximum_marks', {required: true, valueAsNumber: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
                <div><label className="block text-sm mb-1">Passing Marks</label><input type="number" {...register('passing_marks', {required: true, valueAsNumber: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm mb-1">Weightage (%)</label><input type="number" {...register('weightage', {required: true, valueAsNumber: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
