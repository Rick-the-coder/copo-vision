...
const PSOsPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['program-specific-outcomes'], queryFn: async () => (await api.get('/program-specific-outcomes')).data });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/program-specific-outcomes', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['program-specific-outcomes'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/program-specific-outcomes/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['program-specific-outcomes'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/program-specific-outcomes/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['program-specific-outcomes'] }); }
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800">Program Specific Outcomes (PSO)</h2>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Program Specific Outcomes (PSO)</h3>
