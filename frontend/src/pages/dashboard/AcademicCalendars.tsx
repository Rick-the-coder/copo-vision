...
const AcademicCalendarsPage = () => {
  const queryClient = useQueryClient();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AcademicCalendar | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  const { register, handleSubmit, reset } = useForm<any>();

  const { data: items, isLoading } = useQuery({ queryKey: ['academic-calendars'], queryFn: async () => (await api.get('/academic-calendars')).data as AcademicCalendar[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/academic-calendars', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['academic-calendars'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/academic-calendars/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['academic-calendars'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/academic-calendars/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['academic-calendars'] }); }
  });

  const openModal = (item: AcademicCalendar | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800">Academic Calendars</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-100 text-sm uppercase tracking-wider text-slate-600">
              <tr>
                <th className="p-4">Academic Year ID</th>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Calendar Event</h3>
...
export default AcademicCalendarsPage;
