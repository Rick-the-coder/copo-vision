...
const AssessmentWeightagePage = () => {
  const queryClient = useQueryClient();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AssessmentWeightage | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  const { register, handleSubmit, reset } = useForm<any>();

  const { data: items, isLoading } = useQuery({ queryKey: ['assessment-weightages'], queryFn: async () => (await api.get('/assessment-weightages')).data as AssessmentWeightage[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/assessment-weightages', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-weightages'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/assessment-weightages/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-weightages'] }); closeModal(); }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/assessment-weightages/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['assessment-weightages'] }); }
  });

  const openModal = (item: AssessmentWeightage | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
    else reset({ is_active: true });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800">Assessment Weightage</h2>
        <button onClick={() => openModal()} className="bg-primary text-white px-4 py-2 rounded-lg flex items-center gap-2">
          <Plus className="w-4 h-4" /> Add Weightage
        </button>
      </div>
              ...
              {isLoading ? <tr><td colSpan={4} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4">{item.assessment_type_id}</td>
                    <td className="p-4">{item.course_id}</td>
                    <td className="p-4 font-semibold">{item.weightage_percentage}%</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Weightage</h3>
...
export default AssessmentWeightagePage;
