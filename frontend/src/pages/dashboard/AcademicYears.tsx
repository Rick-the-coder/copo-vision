...
const AcademicYearsPage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/academic-years', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['academic-years'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/academic-years/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['academic-years'] }); closeModal(); }
  });
              ...
              {isLoading ? <tr><td colSpan={4} className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary" /></td></tr> :
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-4">{item.academic_year}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Academic Year</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Academic Year</label><input {...register('academic_year', {required: true})} className="w-full px-3 py-2 border rounded-lg" placeholder="2023-2024" /></div>
              <div><label className="block text-sm mb-1">Start Date</label><input type="date" {...register('start_date')} className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">End Date</label><input type="date" {...register('end_date')} className="w-full px-3 py-2 border rounded-lg" /></div>
