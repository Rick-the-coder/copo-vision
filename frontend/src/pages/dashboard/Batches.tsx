...
const BatchesPage = () => {
  ...
  const { data: items, isLoading } = useQuery({ queryKey: ['batches'], queryFn: async () => (await api.get('/batches')).data as Batch[] });

  const createMutation = useMutation({
    mutationFn: (newItem: any) => api.post('/batches', newItem),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['batches'] }); closeModal(); }
  });

  const updateMutation = useMutation({
    mutationFn: (data: { id: number, item: any }) => api.put(`/batches/${data.id}`, data.item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['batches'] }); closeModal(); }
  });
  ...
  const openModal = (item: Batch | null = null) => {
    setEditingItem(item);
    if (item) reset(item);
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{editingItem ? 'Edit' : 'Add'} Batch</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div><label className="block text-sm mb-1">Name</label><input {...register('batch_name', {required: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm mb-1">Year</label><input type="number" {...register('batch_year', {required: true, valueAsNumber: true})} className="w-full px-3 py-2 border rounded-lg" /></div>
